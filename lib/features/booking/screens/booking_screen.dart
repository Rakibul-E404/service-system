...
///
///
/// todo::::::: fixing to show a warning message for without login
///
///
///
///
...
class _BookingScreenState extends State<BookingScreen> with SingleTickerProviderStateMixin {
  ...
  final PendingBookingsController pendingBookingsController = Get.put(PendingBookingsController());
  final CompletedBookingsController completedBookingsController = Get.put(CompletedBookingsController());
  final ProfileService profileService = Get.find<ProfileService>(); // Get ProfileService
  ...
  void initState() {
    ...
    // Fetch initial data for first tab if user is logged in
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: SafeArea(
        child: Obx(() {
          // CHECK IF USER IS LOGGED IN - SAME AS PROFILE SCREEN
          if (!profileService.isLoggedIn.value) {
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
