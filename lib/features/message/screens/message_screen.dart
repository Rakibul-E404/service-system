...
class _MessageScreenState extends State<MessageScreen> {
  final MessageController controller = Get.put(MessageController());
  final ProfileService profileService = Get.find<ProfileService>(); // Add ProfileService
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Obx(() {
          ...
          // USER IS LOGGED IN - SHOW MESSAGES
          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[
              // Search Bar
  ...
  Widget _buildEmptyState() {
    return RefreshIndicator(
      onRefresh: () => profileService.isLoggedIn.value
          ...
          child: Center(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 20),
                ElevatedButton(
                  onPressed: () => profileService.isLoggedIn.value
                      ? controller.forceRefreshConversations()
                      : null,
                  style: ElevatedButton.styleFrom(
                    backgroundColor: AppColors.primaryColor,
