...
class InquiryBottomSheetState extends State<InquiryBottomSheet> {
  ...
  // Static address data for suggestions
  final List<String> _addressSuggestions = const [
    'Dhaka, Bangladesh',
    'Dhaka City, Bangladesh',
    'Dhaka Division, Bangladesh',
    'Dhaka Cantonment, Bangladesh',
  ...
  void initState() {
    ...
    // Initially show all suggestions (always visible requirement)
    _filteredSuggestions = List<String>.from(_addressSuggestions);
  }
  ...
  void _filterSuggestions(String query) {
    if (query.isEmpty) {
      setState(() {
        // Always show all suggestions when query is empty
        _filteredSuggestions = List<String>.from(_addressSuggestions);
      });
      return;
    }
  ...
  Future<void> submitInquiry({
    ...
    try {
      ...
      // From this refactor, always use booking flow with serviceId
      debugPrint('📋 Submitting BOOKING');
      final String finalServiceId = serviceId ?? widget.preSelectedServiceId!;
      final DateTime finalDate =
          selectedDate ?? widget.preSelectedDate ?? DateTime.parse(widget._dateTEController.text.trim());
      final String finalTime = selectedTime ?? widget.preSelectedTime ?? '10:00';
      final String formattedDateTime = _formatDateTime(finalDate, finalTime);
      ...
      if (result['success'] == true) {
        ...
        setState(() {
          ...
          _catId = null;
          _catName = null;
          // keep suggestions visible as full list
          _filteredSuggestions = List<String>.from(_addressSuggestions);
        });
    ...
    return Scrollbar(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: <Widget>[
            /// CATEGORY + SUB-CATEGORY (always visible)
            CategorySubCategoryPicker(
              onCategoryChanged: (name, id) {
                setState(() {
                  _catName = name;
            ...
            /// LOCATION DROPDOWN (always enabled)
            AppCustomContainerField(
              containerChild: DropdownButtonFormField<String>(
                value: _selectedLocation,
            ...
            /// ADDRESS FIELD (always visible) + SUGGESTIONS LIST (always in tree)
            Container(
              width: double.infinity,
              decoration: BoxDecoration(
                color: AppColors.whiteColor,
            ...
            // Suggestions list always present under field
            _buildSuggestionsList(),

            const SizedBox(height: AppSizes.md),

            /// DATE PICKER FIELD (always visible)
            GestureDetector(
              onTap: () async {
                final DateTime now = DateTime.now();
                final DateTime? pickedDate = await showDatePicker(
                  context: context,
                  initialDate: widget.preSelectedDate ?? now,
                  firstDate: now,
                ...
                if (pickedDate != null) {
                  ...
                  debugPrint(
                      '📅 Date Selected: ${widget._dateTEController.text}');
                }
              },
              child: AbsorbPointer(
                child: Container(
            ...
            /// ADDITIONAL NOTE (always visible)
            Container(
              width: double.infinity,
              decoration: BoxDecoration(
                color: AppColors.whiteColor,
            ...
            /// SUBMIT BUTTON (you can place it here or outside bottom sheet)
            SizedBox(
              width: double.infinity,
            ...
            /// LOADING INDICATOR AREA (always visible, spinner toggles)
            SizedBox(
              height: 60,
              child: Center(
                child: _isSubmitting
