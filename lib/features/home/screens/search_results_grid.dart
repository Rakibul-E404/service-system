  ...
  Widget build(BuildContext context) {
    return GetBuilder<HomeSearchController>(
      builder: (HomeSearchController ctrl) {
        ...
        final bool hasResults = ctrl.filteredServices.isNotEmpty;
        final bool shouldShowSeeAll = hasResults && ctrl.filteredServices.length > initialItemsPerPage;
...
class SearchResultsGrid extends StatelessWidget {
  ...
  List<dynamic> get _displayServices {
    if (showAllServices) {
      ...
      return controller.filteredServices.take(initialItemsPerPage).toList();
    }
  }
  ...
  Widget _buildServicesGrid(HomeSearchController ctrl) {
    return SliverPadding(
      ...
      sliver: SliverGrid(
        ...
        delegate: SliverChildBuilderDelegate(
              (BuildContext context, int index) {
            final item = _displayServices[index] as Map<String, dynamic>;
            return _buildServiceCard(item);
          },
          childCount: _displayServices.length,
        ),
      ),
    );
  }
