...
class SubCategoriesPage extends GetView<SubCategoriesController> {
  ...
  Widget build(BuildContext context) {
    ...
    final dynamic args = Get.arguments;
    final String initialCategoryId = (args is Map<String, dynamic>)
        ? (args['categoryId']?.toString() ?? '')
        : '';
    final String initialCategoryName = (args is Map<String, dynamic>)
        ? (args['categoryName']?.toString() ?? 'Category')
        : 'Category';

    // Initialize controller and local state
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (controller.categoryId.value.isEmpty && initialCategoryId.isNotEmpty) {
        controller.setCategory(initialCategoryId, initialCategoryName);
        _selectedCategoryId.value = initialCategoryId;
        _selectedCategoryName.value = initialCategoryName;
      }
    });
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return GestureDetector(
                            onTap: () {
                              // Update controller with new category
                              controller.setCategory(category.id, category.name);
