...
class PersonalInformationScreen extends GetView<ProfileInformationController> {
  ...
  Widget build(BuildContext context) {
    ...
    // Get ProfileService for reactive updates
    ...
    return Scaffold(
      ...
      body: Obx(() {
        ...
        return controller.isEditing.value
            ? _buildEditMode(controller, context)
            : _buildViewMode(controller, profileService);
      }),
    );
  }

  // View Mode
  Widget _buildViewMode(ProfileInformationController controller, ProfileService profileService) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(24),
      child: Column(
        children: <Widget>[
          ...
          // Profile Picture - Using ProfileService for real-time updates
          Obx(() {
            final imageUrl = controller.getImageUrl();
            final isLocal = controller.isLocalImage();
          ...
          SizedBox(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                // Name - Using ProfileService directly
                Obx(() => _buildInfoItem(
                  icon: Icons.person_outline,
                  label: 'Name',
                  value: profileService.name.value.isNotEmpty
                ...
                // Email - Using ProfileService directly
                Obx(() => _buildInfoItem(
                  icon: Icons.email_outlined,
                  label: 'Email',
                  value: profileService.email.value.isNotEmpty
                ...
                // Location - Using ProfileService directly
                Obx(() => _buildInfoItem(
                  icon: Icons.location_on_outlined,
                  label: 'Location',
                  value: profileService.getDisplayLocation(),
                  iconColor: Colors.amber[600]!,
                )),

                const SizedBox(height: 20),

                // Role - Using ProfileService directly
                Obx(() => _buildInfoItem(
                  icon: Icons.badge_outlined,
                  label: 'Role',
                  value: profileService.role.value.isNotEmpty
                      ? profileService.role.value[0].toUpperCase() +
                      profileService.role.value.substring(1)
                      : 'Not provided',
                  iconColor: Colors.amber[600]!,
                )),
              ],
        ...
        children: <Widget>[
          ...
          Obx(() {
            ...
            return SizedBox(
              width: double.infinity,
              child: ElevatedButton(
                onPressed: profileService.isLoading.value
                    ? null
                    : () => controller.updateProfile(),
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.amber[600],
                ...
                child: profileService.isLoading.value
                    ? const SizedBox(
                  height: 20,
                  width: 20,
                  child: CircularProgressIndicator(
