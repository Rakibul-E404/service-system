...
///
///
/// todo::: navigating to the phone and the email
///
///
///
...
class UserProviderProfileController extends GetxController {
  ...
  // Getter for phone number from profile
  String get phoneNumber => profile?.phone ?? '';

  // Getter for email from profile author
  String get email => profile?.author.email ?? '';

  // Getter for accessible contact methods from ProviderInfoData
  List<String> get accessibleContacts => providerData.value?.accessibleBySubscription ?? [];

  bool get hasData => providerData.value != null;

  // Helper methods to check specific access
  bool get canCall => accessibleContacts.contains('Call');
  bool get canEmail => accessibleContacts.contains('Email');
}