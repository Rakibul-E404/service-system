...
///
///
/// todo::::::::: adding the resend functionality
///
///
///
///
...
class VerifyEmailController extends GetxController {
  ...
  // Store all sign up data for resend
  ...
  /// Resend OTP - Hit SignUp API again if from signup, else hit forgot password API
  Future<void> resendOtp() async {
    if (userEmail.value.isEmpty) {
      Get.snackbar(
        'Error',
    ...
    try {
      ...
      if (fromForgotPassword) {
        // For forgot password, just send email
        apiUrl = AppUrl.forgotPassword;
        requestBody = {
          'email': userEmail.value,
        };
        debugPrint('Using Forgot Password API');
      } else {
        // For sign up, send all registration data
        if (userName.value.isEmpty || userLocation.value.isEmpty ||
            userPassword.value.isEmpty || userRole.value.isEmpty) {
          Get.snackbar(
            'Error',
            'Registration data not found. Please go back and sign up again.',
            backgroundColor: Colors.red,
            colorText: Colors.white,
            snackPosition: SnackPosition.TOP,
          );
      ...
      debugPrint('📥 Resend OTP Response: ${response.jsonResponse}');
      isLoading.value = false;
      ...
      if (response.isSuccess) {
        ...
        if (success) {
          // Extract new OTP if available
          if (!fromForgotPassword) {
            final data = response.jsonResponse?['data'];
            final newOtp = data?['oneTimeCode']?.toString();
            if (newOtp != null) {
              receivedOtp = newOtp;
              debugPrint('🔐 New OTP received: $newOtp');
            }
          }
          ...
          Get.snackbar(
            'Error',
            response.jsonResponse?['message'] ?? 'Failed to resend OTP',
            backgroundColor: Colors.red,
            colorText: Colors.white,
            snackPosition: SnackPosition.TOP,
          );
  ...
  void onInit() {
    ...
    if (arguments != null) {
      if (arguments is Map<String, dynamic>) {
        ...
        userLocation.value = arguments['location'] ?? '';
        userPassword.value = arguments['password'] ?? '';
        userRole.value = arguments['role'] ?? '';
        receivedOtp = arguments['otp'] ?? '';
        fromForgotPassword = arguments['fromForgotPassword'] ?? false;
      } else if (arguments is String) {
        userEmail.value = arguments;
    ...
    debugPrint('📧 User Email: ${userEmail.value}');
    debugPrint('👤 User Name: ${userName.value}');
    debugPrint('📍 User Location: ${userLocation.value}');
