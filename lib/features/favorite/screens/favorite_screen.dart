            ...
            delegate: SliverChildBuilderDelegate(
                ...
                return Padding(
                  ...
                  child: HorizontalServiceCard(
                    ...
                    status: 'Favorite',
                    tabIndex: 3,
                    showStatus: false, // ← This hides the "Favorite" badge
                    onTap: () {
                    ...
                    onDelete: () async {
                      ...
                      if (confirmDelete == true) {
                        ...
                        try {
                          ...
                          final success = await controller.removeFavorite(favoriteId);
...
class FavoriteScreen extends GetView<FavoriteController> {
  ...
  Widget _buildContent(BuildContext context, HomeTopBarController profileController) {
    // Fetch favorites when screen first loads
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (controller.favorites.isEmpty && !controller.isLoading.value) {
        debugPrint('🔄 FavoriteScreen: Initial fetch triggered');
        controller.fetchFavorites(refresh: true);
      }
    });
        ...
        SliverToBoxAdapter(
          child: Column(
            ...
            children: [
              ...
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: AppSizes.md),
                child: Text('My Favorites', style: context.txtTheme.titleLarge),
              ),
              const SizedBox(height: AppSizes.md),
            ],
          ),
        ...
        Obx(() {
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: () {
                          Get.back(); // Go back to services page
                        },
          ...
          return SliverList(
            delegate: SliverChildBuilderDelegate(
                ...
                // Debug info
                debugPrint('🔄 Building favorite item $index');
                debugPrint('   - Favorite ID: ${favorite.id}');
                ...
                return Padding(
                  ...
                  child: HorizontalServiceCard(
                    ...
                    title: service.title ?? 'Service',
                    subtitle: service.subcategoryName ?? 'Category',
                    description: service.description ?? (service.location ?? 'Location not specified'),
                    bookingId: favorite.id ?? '',
                    status: 'Favorite',
                    tabIndex: 3,
                    showStatus: false,
                    ...
                    onDelete: () async {
                      await _showDeleteConfirmationDialog(context, favorite, service);
                    },
                  ),
                );
              },
  ...
  Future<void> _showDeleteConfirmationDialog(
      BuildContext context, FavoriteModel favorite, ProviderService service) async {
    final bool? confirmDelete = await showDialog<bool>(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
    ...
    if (confirmDelete == true) {
      await _deleteFavorite(favorite, service);
    }
  }

  // Delete favorite and update UI
  Future<void> _deleteFavorite(FavoriteModel favorite, ProviderService service) async {
    Get.dialog(
      const Center(
        child: CircularProgressIndicator(),
      ),
    ...
    try {
      ...
      if (favoriteId == null || favoriteId.isEmpty || serviceId == null || serviceId.isEmpty) {
        Get.back();
        Get.snackbar(
          'Error',
          'Unable to remove favorite: Invalid favorite ID',
      ...
      debugPrint('🗑️ Deleting favorite:');
      debugPrint('   - Favorite ID: $favoriteId');
      debugPrint('   - Service ID: $serviceId');
      debugPrint('   - Service Title: ${service.title}');

      // Use the toggleFavorite method instead of removeFavorite to ensure consistency
      // This will handle both server API call and local state updates
      await controller.toggleFavorite(serviceId);
      ...
      Get.back();
      debugPrint('❌ Exception deleting favorite: $e');
