      ...
      if (response.isSuccess && response.jsonResponse != null) {
        ...
        if (responseData['success'] == true && responseData['data'] != null) {
          ...
          if (dataList.isEmpty) {
            ...
            for (var favData in dataList) {
              try {
                // CORRECT: Extract service ID from the 'service' field (not from profile)
                ...
                if (serviceId != null && serviceId.isNotEmpty) {
                  // Create ProviderService object
                  final providerService = ProviderService(
                    id: serviceId, // Use the service ID from 'service' field
                    title: profileData['businessName']?.toString() ?? 'No Title',
                    description: profileData['description']?.toString() ?? '',
                    image: profileData['image']?.toString(),
                    location: profileData['location']?.toString() ?? 'Unknown Location',
                  ...
                  final favorite = FavoriteModel(
                    id: favData['_id']?.toString() ?? '',
                    userId: profileData['author']?.toString(),
                    providerServiceId: serviceId, // Store the SERVICE ID
                    providerService: providerService,
                    isDeleted: false,
                  );
                  ...
                  // Update favorite status maps with SERVICE ID
                  favoriteStatus[serviceId] = true;
                  serviceToFavoriteId[serviceId] = favorite.id;

                  developer.debugPrint('✅ Service ID from "service" field: $serviceId');
                  developer.debugPrint('✅ Profile ID from profile._id: ${profileData['_id']}');
                  developer.debugPrint('✅ Marked service $serviceId as favorited');
                } else {
                  developer.debugPrint('⚠️ Skipping favorite item with no service ID');
                  developer.debugPrint('📋 Raw data: service field = ${favData['service']}');
                }
              } catch (e) {
                developer.debugPrint('❌ Error parsing favorite item: $e');
                developer.debugPrint('📋 Failed data: $favData');
